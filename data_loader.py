...
def dataloader_origin_test(base_path):
    ...
    train_dataset, test_dataset= torch.utils.data.random_split(origin_dataset, [n-n_test, n_test])
...
def dataLoader_cross(origin_dataset, train_idx, val_idx):
    ...
    valid_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=False, num_workers=1)
...
def dataLoaderLOPO(base_path, leave_out_id):
    ...
    # Create data loaders
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
    valid_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=True, num_workers=1)
